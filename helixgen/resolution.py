...
def sample_helix_resolution(rng: jax.random.PRNGKey, hel: Helix)\
        -> (Helix, dtype):
    """ Helix resolution sampler """
    return sample_resolution(rng, hel, helix_covariance)


def sample_position_resolution(rng: jax.random.PRNGKey, pos: Position)\
        -> (Position, dtype):
    """ Position resolution sampler """
    return sample_resolution(rng, pos, position_covariance)


def sample_momentum_resolution(rng: jax.random.PRNGKey, mom: Momentum)\
        -> (Momentum, dtype):
    """ Momentum resolution sampler """
    return sample_resolution(rng, mom, momentum_covariance)


def sample_cluster_resolution(rng: jax.random.PRNGKey, clu: Cluster)\
        -> (Cluster, np.ndarray):
...
def apply_resolution_charged(rng: jax.random.PRNGKey, pos: Position,
                             mom: Momentum, pcl: Particle)\
        -> (Helix, np.ndarray, Position, Momentum):
    ...
    pos, mom = helix_to_cartesian(hel, length=0., q=q, B=B)
    return {
        'hel': hel,
...
def apply_resolution_neutral_cartesian(rng: jax.random.PRNGKey, pos: Position,
                                       mom: Momentum)\
...
def apply_resolution_neutral(rng: jax.random.PRNGKey, pos: Position,
                             mom: Momentum)\
        -> (Cluster, np.ndarray, Momentum):
...
def apply_resolution_cartesian(rng: jax.random.PRNGKey, genpcls: dict)\
        -> (dict):
    return apply_resolution(rng, genpcls,
